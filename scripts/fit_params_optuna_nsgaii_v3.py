...
1. Предзагрузка всех реранкеров перед оптимизацией (экономия 20-40 сек на trial)
2. Прогрев embedding модели
3. Улучшенное логирование времени загрузки
4. Защита от повторной загрузки моделей
5. Профессиональные графики для диссертации:
   - Эволюция всех трёх метрик (CR, Faith, AR) по поколениям
   - Матрица корреляций параметров и метрик
   - Улучшенные boxplot и Парето-фронт
   - Сохранение в PDF + PNG
"""
import csv
import json
import logging
...
from rag_gen import Settings, SmetaRAGApp, cosine_similarity, get_cached_reranker, collect_sources, docs_to_context
